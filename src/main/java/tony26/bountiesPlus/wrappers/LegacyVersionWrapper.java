...
public class LegacyVersionWrapper implements VersionWrapper {
    ...
    public ItemStack createPlayerHead(String playerName) {
        ...
        try {
            skullMeta.setOwner(playerName); // Sets skull owner for legacy skin application (1.8.8–1.12.2)
        ...
        BountiesPlus.getInstance().getLogger().info("Successfully created player head for " + playerName + " (Legacy)"); // Logs successful creation
     ...
     * // note: Adds DURABILITY enchantment with HIDE_ENCHANTS flag for 1.8.8 compatibility
     */
    @Override
    public void applyGlow(ItemStack item, boolean glowEnabled) {
        ...
        if (meta == null) {
            BountiesPlus.getInstance().getDebugManager().logWarning("[DEBUG - LegacyVersionWrapper] Failed to get ItemMeta for glow on item " + item.getType().name());
            return;
        }
        meta.addEnchant(Enchantment.DURABILITY, 1, true);
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        item.setItemMeta(meta);
        BountiesPlus.getInstance().getDebugManager().logDebug("[DEBUG - LegacyVersionWrapper] Applied glow to item " + item.getType().name());
    }
    ...
    public void sendTitle(Player player, String title, String subtitle, int fadeIn, int stay, int fadeOut) {
        // Legacy versions don't support titles with timing, use basic version
    ...
    /**
     * Gets version-safe material for legacy versions
     */
    @Override
    public Material getMaterial(String materialName) {
        ...
        if (material == Material.STONE && !materialName.equalsIgnoreCase("STONE")) {
            BountiesPlus.getInstance().getLogger().warning("Invalid material '" + materialName + "' in legacy version, using STONE"); // Logs warning
        }
    ...
    public String getSuccessSound() {
        return "ORB_PICKUP"; // Legacy sound name
    }

    @Override
    public String getErrorSound() {
        return "VILLAGER_NO"; // Legacy sound name
    }
}