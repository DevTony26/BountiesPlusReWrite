...
public class SkullUtils {
    ...
    public static ItemStack createVersionAwarePlayerHead(OfflinePlayer targetPlayer) {
        ...
        if (playerName == null || playerName.isEmpty()) {
            ...
            return createFallbackSkull(targetPlayer); // Returns skeleton skull if no name
        }
        ...
        if (head == null || head.getType() == Material.STONE) {
            ...
            return createFallbackSkull(targetPlayer); // Returns skeleton skull if material fails
        }
        ...
        if (VersionUtils.isLegacy()) {
            if (head.getType() != Material.SKULL_ITEM || head.getDurability() != 3) {
                ...
                return createFallbackSkull(targetPlayer); // Returns skeleton skull for invalid legacy skull
            }
        } else {
            if (!head.getType().name().equals("PLAYER_HEAD")) {
                ...
                return createFallbackSkull(targetPlayer); // Returns skeleton skull for invalid modern skull
            }
        }
        ...
        if (!(rawMeta instanceof SkullMeta)) {
            ...
            return createFallbackSkull(targetPlayer); // Returns skeleton skull if meta is invalid
        }
        SkullMeta skullMeta = (SkullMeta) rawMeta;
        try {
            if (!VersionUtils.isLegacy()) {
                // Attempt 1: setOwningPlayer via reflection for modern versions
                try {
                    ...
                    if (isSkinApplied(skullMeta, playerName)) {
                        ...
                        return head; // Returns head with skin applied
                    }
            ...
            // Attempt 2: setOwner for legacy and modern fallback
            ...
            // Attempt 3: VersionWrapper fallback
            ...
            if (skinnedHead != null && VersionUtils.isPlayerHead(skinnedHead)) {
                ...
                if (skinnedMeta != null && isSkinApplied(skinnedMeta, playerName)) {
                    ...
                    return skinnedHead; // Returns head with skin applied
                }
        ...
        return createFallbackSkull(targetPlayer); // Returns skeleton skull if all skin attempts fail
    }
    ...
    public static boolean isValidBountySkull(ItemStack item) {
        ...
        if (displayName != null && displayName.contains("Bounty Head")) {
            return true;
        }
